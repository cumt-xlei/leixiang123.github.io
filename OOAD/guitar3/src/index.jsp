...
<script>
	function add(){
		...
		if(builder != "" && model != "" && type != "" && backWood != "" && topWood != ""){
			...
			if(price != null && price != ""){
				...
				//打开ajax请求
				xmlhttp.open("GET","addInventory.do?builder="+builder+"&model="+model+"&type="+type+"&backWood="+back+"&topWood="+top+"&price="+price,true);
				//发送请求
				xmlhttp.send();
	...
	function search(){
		...
		xmlhttp.onreadystatechange = function(){
			if(xmlhttp.readyState == 4 ) {       //完全得到服务器的响应  
				if(xmlhttp.status == 200) {      //没有异常  
					...
					//每次查询时把table初始化
					tab.innerHTML="<tr><th>serialNumber</th><th>builder</th><th>model</th><th>type</th><th>backWood</th><th>topWood</th><th>price</th></tr>";
					//循环把查询结果插入table
					for(var i = 0;i < len;i++){
						//创建tr节点
						...
						//为td节点赋值
						newTd1.innerHTML = json[i].serialNumber;
						newTd2.innerHTML = json[i].builder;
						newTd3.innerHTML = json[i].model;
						newTd4.innerHTML = json[i].type;
						newTd5.innerHTML = json[i].backWood;
						newTd6.innerHTML = json[i].topWood;
						newTd7.innerHTML = json[i].price;
		...
		//打开ajax请求
		xmlhttp.open("GET","getInventory.do?builder="+builder+"&model="+model+"&type="+type+"&backWood="+back+"&topWood="+top,true);
		//发送请求
		xmlhttp.send();
	}
</script>
